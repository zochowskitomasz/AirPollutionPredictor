...
After that aforementioned initial case, we moved on to scaling it up and setting up a live predictor. That was achieved on an old RaspberryPi. Here a baseline was developed, and the workings of the APIs became clearer (as described [here](../archive/raspberrypi/README.md)). We quickly understood that in order to store a larger amount of historical data we would require something with more computing power. Luckily, and old gaming laptop found its way into our hands. There, we set up an ubuntu server `tartaros` and started working on moving the project there.
...
After setting up the `ubuntu` server, another issue was connected to port funneling and exposing the server `tartaros` to the Internet. That was solved by using a VPN, which allows us to ssh into the machine from any laptop that has said VPN installed. Another infrastructural issue we encountered was `Airflow`. At first, we planned for our project to be managed with and `Airflow DAG`, connecting all the tasks into a nice graph. However, after a long and tiring battle with the `Airflow` software, we ended up deprecating that idea. Instead, we switched to `cron`, which luckily worked immediately, and put everything into motion. Despite the switch, we kept the `postgres` user `airflow` as a monument to that lost battle of installation.
